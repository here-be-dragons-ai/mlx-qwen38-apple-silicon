...
| `KV_BITS=8 QUANT_KV_START=8192` | **−4.2 GiB** | **it did nothing at all until 2026-09-07.** `make_speculative_prompt_cache` dropped the cache factory at `batch_size == 1`, so any server with a drafter got dense caches and no warning (upstream `#2093`, confirmed here from our own snapshot headers). `mlx-vlm 0.7.0` ships `#1822` and removes that bypass, so the lever exists now. Two things before enabling it: quantised caches never reach the fused kernels of patch `0013`, and `#2163` reports that path peaking **higher than f16 below ~50k** on this model. The ceiling predicts +12 % decode at 65k. Measure; do not default |
